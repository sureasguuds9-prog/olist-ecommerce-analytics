...
## Логика витрины и расчётов

- **Гранулярность:** одна строка в `olist.order_mart` соответствует одному `order_id`.
- **Защита от размножения строк:** платежи, позиции и отзывы сначала агрегируются в отдельных CTE до уровня заказа и только затем присоединяются к `orders`.
- **Статусы:** основные бизнес-метрики рассчитываются по `delivered`; отменённые и недоставленные заказы остаются доступными для проверок качества, но не попадают в delivered-KPI.
- **Клиент:** для повторных покупок используется `customer_unique_id`, а не технический `customer_id` конкретного заказа.
- **Первая покупка:** минимальная дата доставленного заказа клиента.
- **Retention:** месяц первой покупки задаёт когорту, разница календарных месяцев — `cohort_index`, доля считается от размера когорты в нулевом месяце.

## Проверки после JOIN

После построения витрины обязательно сверяются:
...
Ожидаемый результат: `99 441` строк и `99 441` уникальных заказов. Если значения расходятся, аналитические суммы и доли использовать нельзя.
